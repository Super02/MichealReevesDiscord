...
async def on_ready(): # On ready
	await bot.change_presence(game=discord.Game(name="-help")) # Set playing game to -help
	print ("Bot ready") # Print information to console
	print ("I am running on " + bot.user.name)
	print ("With the ID: " + bot.user.id)

@bot.command(pass_context=True) # Simple ping command
async def ping(ctx):
	await bot.say(":ping_pong: pong!")
@bot.command(pass_context=True) # User information command
async def info(ctx, user: discord.Member):
	embed = discord.Embed(title="{}'s info".format(user.name), description="This is what I could find...", color=0x00ff00) # Creating embed
	...
	await bot.say(embed=embed) # Sending embed
@bot.command(pass_context=True) # Server information command
async def serverinfo(ctx):
	embed = discord.Embed(title="{}'s info".format(ctx.message.server.name), description="Here is what I could find...", color=0x00ff00) # Creating embed
...
async def kick(ctx, user: discord.Member):
	if(ctx.message.author.id == "305246941992976386" or ctx.message.author.server_permissions.kick_members and user.id != "305246941992976386"): # Checking for permissions
		try:
			await bot.kick(user)
...
@bot.command(pass_context=True) # Ban command
async def ban(ctx, user: discord.Member):
	if(ctx.message.author.id == "305246941992976386" or ctx.message.author.server_permissions.ban_members and user.id != "305246941992976386"): # Checking for permissions
		try:
			await bot.ban(user)
		...
		embed.add_field(name="Permission", value="denied", inline=True)
		await bot.say(embed=embed)
@bot.command(pass_context=True) # Unban command 
async def unban(ctx, user: discord.Member):
	if(ctx.message.author.id == "305246941992976386" or ctx.message.author.server_permissions.ban_members): # Checking for permissions
		try:
		...
		embed.add_field(name="Permission", value="denied", inline=True)
		await bot.say(embed=embed)
@bot.command(pass_context=True) # Say command 
async def say(ctx, *, arg):
	if(ctx.message.author.id == "305246941992976386" or ctx.message.author.server_permissions.administrator): # Checking for permissions
		await bot.say(arg)
	else:
...
async def hexcolor(ctx, *, arg):
	if(arg == "nigger" or arg == "nigga"):
		...
		try:
			...
			print(e)
@bot.command(pass_context=True, aliases=['purge', 'prune']) # Clear/Prune command bulk deletes text messages
async def clear(ctx, amount=None):
	try:
		...
		async for msg in bot.logs_from(ctx.message.channel,limit=2): 
			msgs.append(msg) # Deleting command and error messages
		await asyncio.sleep(3)
		await bot.delete_messages(msgs)
		return
	
	if(ctx.message.author.id == "305246941992976386" or ctx.message.author.server_permissions.manage_messages): # Checking for permissions
		msgs=[]
...
@bot.command(pass_context=True) # Setgame command
...
bot.run("Sorry to say. You're not getting my Discord bot token xD") # Running of my bot token
